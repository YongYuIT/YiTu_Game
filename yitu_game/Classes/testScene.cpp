...
bool testScene::init()
{
	...
	for (int i = 1; i <= 16; i++)
	{
		cocos2d::log("--------->%d", i);
		int weishi = i < 10 ? 1 : 2;
		char* num = add_char_string("sp_", getNum(i, weishi), 3, weishi);
	...
	btn_start->addTouchEventListener(this, toucheventselector(testScene::on_btn_start_click));
	Button* btn_restart = (Button*)this->rootNode->getChildByName("lay_diffcult_menu")->getChildByName("btn_restart");
	btn_restart->addTouchEventListener(this, toucheventselector(testScene::on_btn_restart_click));

	addChild(rootNode);
	return true;
}

void testScene::auto_move_next(int current_steps, int total_steps, int last)
{
	...
	for (int j = 1; j < 17; j++)
	{
		if (sps[j]->getBoundingBox().containsPoint(point_next))
		{
			...
			Point visable = sps[j]->getPosition();
			auto back_func = [this, current_steps, total_steps, num]()
			{
				int currrent = current_steps + 1;
				this->auto_move_next(currrent, total_steps, num);
			};
			CallFunc* call_back = CallFunc::create(back_func);
			sps[4]->runAction(MoveTo::create(0, visable));
			sps[j]->runAction(Sequence::create(MoveTo::create(0.3, unvisable), DelayTime::create(0.2), call_back, NULL));
...
void testScene::on_btn_start_click(Ref*, ui::Widget::TouchEventType type)
{
...
void testScene::on_btn_restart_click(Ref*, ui::Widget::TouchEventType type)
{
	for (int i = 1; i <= 16; i++)
	{
		sps[i]->setPosition(original_position[i]);
		this->rootNode->getChildByName("txt_success")->setVisible(false);
	}
}

void testScene::game_start(int steps)
{
	//使右上角的精灵闪烁后消失----------------start
