...
bool MainScene::init()
{
	...
	for (int i = 1; i <= 16; i++)
	{
		cocos2d::log("--------->%d", i);
		int weishi = i < 10 ? 1 : 2;
		char* num = add_char_string("sp_", getNum(i), 3, weishi);
...
void MainScene::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
	...
	if (keyCode == EventKeyboard::KeyCode::KEY_BACK)//back键
	{
		...
		//bool isHave = JniHelper::getStaticMethodInfo(info, "org/cocos2dx/cpp/AppActivity", "test", "()Ljava/lang/Object;");
		if (isHave)
		{
			auto activityObj = info.env->CallStaticObjectMethod(info.classID, info.methodID);
			cocos2d::log("----------->find!");
			isHave = JniHelper::getMethodInfo(info, "org/cocos2dx/cpp/AppActivity", "do_OnBackDown", "()V");
			if (isHave)
			{
